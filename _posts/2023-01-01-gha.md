...
1. To share values among steps of a job, please use [GITHUB\_ENV](https://docs.github.com/en/actions/learn-github-actions/variables#passing-values-between-steps-and-jobs-in-a-workflow).
   ...
   `GITHUB_ENV` is a Shell feature of the runner. We can access to such variables both with Shell syntax or GHA syntax.
...
2. To share values among jobs, please try the following methods.
   1. Use [GITHUB\_OUTPUT](https://docs.github.com/en/actions/using-jobs/defining-outputs-for-jobs) to declare the output of a job. It is a GHA feature and can only be accessed with GHA syntax.

      ```
      {% raw %}
      # define output1 in job
      output1: ${{ steps.<step-id>.outputs.my_var }}

      # reference the output1 in another job

...
1. Tor perodically chooes different encryption routing nodes. Your Tor IP is the exit node's IP.
...
4. Tor is blocked by GFW:
   ...
   3. Block intermediate rely nodes;
   4. Malicious relay nodes.
...
2. Frontend proxy
   ...
   Application -> Tor -> frontend proxy -> entry node -> intermediate rely -> exit node -> destination host. Traffic between application and exit node is encrypted. Localhost IP is plain between application and frontend proxy.

   Though frontend proxy knows your IP but the traffic is encrypted! Exit nodes and destination host know your traffic but does not know your real IP. What's more, if you visit destination host by *https*, then exit node only knows encrypted *https* traffic.

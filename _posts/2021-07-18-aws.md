...
3. Block Volume. By default, each instance is assigned 50 GB boot volume. However, each account can have up to 200 GB volume. So we can create and attach a block volume to the instance and mount it somewhere (e.g. "/opt"), or extend the LVM.
   1. Create and attach block volume. Please remembr to run the iSCSI commands. Attention that the block volume is attached by network connection:
      ...
      Check [Block Volume](#block-volume) for how to make use of the attached device as LVM.
   2. To mount the new device by "/etc/fstab", remember to add `_netdev` and `nofail` options, as attached block volumes require network access upon boot.
   3. When you "terminate" an instance, remember that the volumes associated remain. OCI would charge you for standaline volumes unless explicitly deleted.
4. Disable all OCI plugins in the instance, like the Custom Logs Monitoring.
...
3. Disable web console control *cockpit.socket*.
4. Firewall. By default, *firewalld* is enabled and most ports are disallowed!

## Block Volume ##

This section demonstrates how to extend the instalce LVM with the block volume.

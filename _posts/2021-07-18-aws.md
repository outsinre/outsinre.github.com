...
1. Create an instance
   ...
   2. Select CentOS 8.2. Now Oracle Linux is recommended!
...
3. Block Volume. By default, each instance is assigned 50 GB boot volume. However, each account can have up to 200 GB volume. 
   ...
   2. Attach the block volume. When you "terminate" an instance, the block volume attached remain. Be careful about the free quota!
   
      The step 3 and step 4 below is dangerous! Avoid it unless required.
   3. Login and run the iSCSI connect commands. Attention that the block volume is network connection:
   ...
   4. Either mount the device somewhere (e.g. "/opt") or [extend it to root LVM](#block-volume). To mount the new device by "/etc/fstab", remember to add `_netdev` and `nofail` options, as attached block volumes require network access upon boot.
   
   CAUTION: if we want to reboot the instance or detach the block volume, please *revert* step 4 and step 3 above, otherwise we lost contact to our instance, even "Cloud Shell" would not help!
   
   ![block volume reminder](/assets/oci-block-volume-reminder.png)

   So, leave the block volume attached and only use it if you are know what you are doing.
4. Optionally, disable OCI plugins, like the Custom Logs Monitoring. OCI plugins communicate with [oracle-cloud-agent.service](#linux-configuration) on instance to collect performance metrics, install OS updates, perform other instance management tasks, etc.
...
3. Enable or disable *cockpit.socket* for Web Console Control (Browser).

   Web Console Control is different than "Could Shell".
...
It is not recommended to extend the block volume into existing LVM as the reverting process is complicated! When rebooting the instance or detach the block volume, we must revert steps below. Check [Instance Configuration](#instance-configuration) for details.

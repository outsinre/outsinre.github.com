...
A Block Directive has the same structure as a simple directive. But the ending semicolon is substituted with a set of additional Instructions (not necessarily Directives) surrounded by braces (`{` and `}`).
...
So, what is the difference between *instruction* and *directive*? The simple answer is [no difference](https://stackoverflow.com/q/47423348). Therefore, we can say a block directive is always a context, and vice versa.
...
The _main_ context is a special context. It is the parent context of everything in the configuration - the outmost context. Directives outside of any contexts are considered to be in the _main_ context. However, the _main_ context are not explicitly named.
...
Once a server context is determined, a location context should be selected.

A general _location_ context is defined as `location optional_modifier uri { }`. To ease ambiguity, we call URI in the definition as *location uri*, while the target as *variable uri*, namely the [normilized](https://stackoverflow.com/a/48709976) *request line* `request_uri`.

Nginx tests location uri against variable uri, but subject to *optional modifier* `= | ~ | ~* | ^~`.

1. `=` means *exact* match. So, localtion uri must be equal to variable uri.
2. `~` (case-sensitive) and `~*` (case-insensitive) do *regex* match.
3. If none of the modifiers are provided, do *prefix* match.
   1. `^~` is a special modifier to *prefix* match. If the current location uri has the *longest* prefix, Nginx skips *regex* match. Hence, we call it *not regex* match.

Here is the matching rule:

1. Firstly, do *exact* match `=`. If matched, terminate the search.
2. Secondly, do *prefix* match and selects the *longest* prefix irrespective of location order in the configuration file. If no location contexts are defined, Nginx assumes the default `location /`. So, we can always get a longest prefix match.
   1. If the longest prefix has modifier `^~`, then terminate the search.
   2. Otherwise, the longest prefix is kept as a candidate.
3. Thirdly, do *regex* match in the order of location context.
   1. The first match terminates the search.
   2. If none of the regex is matched, then the longest prefix in step 2.2 is selected.

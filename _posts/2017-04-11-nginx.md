...
3. *optional_modifier* can be one of `= | ~ | ~* | ^~`. If omited, we it a _prefix_ match. `=` means *exact* match. `~` (case-sensitive) and `~*` (case-insensitive) do *regex* match.
4. `^~` is special. If _uri_ is the longest prefix match, regex match is omited.

To choose _location_ matching a request, nginx:

1. Firstly, do `=` match. If matched, terminate the search.
2. Secondly, do prefix match, irrespective of _location_ order.

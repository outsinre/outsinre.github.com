...
A general _location_ context is defined as `location optional_modifier uri { }`. Nginx tests the *normilized* request line `request_uri`, namely variable [$uri](https://stackoverflow.com/a/48709976) (i.e. `$document_uri`) against *location uri*. To put it simple: test *location uri* against variable `$uri`

The `optional_modifier` can be one of `= | ~ | ~* | ^~`.
...
3. If none of the modifiers are provided, do *prefix* match.
   1. `^~` is a special modifier to *prefix* match. If the location *uri* has the *longest prefix*, Nginx skips *regex* match. Hence, we call it *not regex* match.

Nginx matches *location uri* as follows:
...
2. Secondly, do *prefix* match and selects the *longest prefix* irrespective of location order in the configuration file. If no location contexts are defined, Nginx assumes the default `location /`. So, we can always get a longest prefix match.
   ...
   1. If the longest prefix *uri* has modifier `^~`, then terminate the search.
   2. Otherwise, the longest prefix match is kept as a candidate.
3. Thirdly, do *regex* match in the order of location context.
   1. The first match terminates the search.
   2. If none of the regex is matched, then the longest prefix match in step 2.2 is selected.

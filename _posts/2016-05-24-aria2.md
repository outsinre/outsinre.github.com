...
Make sure `rpc-listen-port` is allowed on *host* by *iptables* (on OpenWRT or PC). If `--rpc-listen-all` is True, *aria2* listens incoming JSON-RPC/XML-RPC requests on all network  interfaces. We can allow incoming requests to a specific interface by `-d host-ip`.
...
### [Public URL](https://ziahamza.github.io/webui-aria2/) and fill in *Settings -> Connection Settings*.
...
3. Use Python *simplehttpserver* to host the cloned webui. RPC listens at 6800 and we can set HTTP Server listens at 6801.
   ...
   Browse *http://localhost:6801* (hosted on access PC) or *http://ip_address_of_server:6801* (hosted on VPS/OpenWRT).
...
4. dht-listen-port: default UDP ports6881-6999.

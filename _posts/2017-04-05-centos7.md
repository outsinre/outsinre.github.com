...
```bash
~ # rpm -ev pkg                     # uninstall a pkg
```

Query:

```bash
~ # rpm -qi pkg; rpm -qa | grep 'regex'               # query pkg information
...
*list updates* is almost the same as *check-update*. As the command form implies, *check-update* is useful in Shell scripting while *list updates* is for humans. Please pay attention, their exit status code difference.
...
CentOS-5 reached the end of its lifecycle as of March 31, 2017. The [baseurl](http://mirror.centos.org/centos/5/readme) is deprecated and moved to a backup location [Vault](http://vault.centos.org/5.8/). Packages there is no longer maintained or upgraded, and may suffer from security risks.

However, we sometimes have to maintain ancient releases for whatever reasons. To install packages through *yum*, [point repositories to the new location](https://www.centos.org/forums/viewtopic.php?t=62130): either manually edit */etc/yum.repos.d/CentOS-Base.repo* or install package *centos-release-5-8.el5.centos.x86_64.rpm*.

...
colima, under the hood, supports Kubernetes via [K3s](https://k3s.io/) which is a lightweight Kubernetes distribution with small memory and CPU footprint (`50%` percent).

To enable Kubernetes, we can add the ['kubernetes' config field](#config-colima). As K3s server would consume extra CPU and memory. Please create a separate [profile](#colima-profile) for K8s specifically. Alternatively, always specifiy the `--kubernetes=true` or `--kubernetes=false` CLI option.
...
If the Kubernetes has been enabled. We can dynamically start/stop the cluster on demand!
...
According to the [FAQ](https://github.com/abiosoft/colima/blob/main/docs/FAQ.md#how-does-colima-compare-to-minikube-kind-k3d), colima Kubernetes does not support multiple clusters! But we can run Minikube, Kind or K3d with colima as the backend. Alternatively, we can create multiple colima instances to simulate multiple K8s clusters.
...
We find a few containers created for different Kubernetes components (e.g. CoreDNS Cluster DNS).

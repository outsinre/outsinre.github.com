...
The default resources assigned to the Linux VM is 2 CPUs, 2 GiB memory and 100 GiB disk space. We want to increase the CPU and memory a bit. Additionally, we want to update more fields as follows. Especially, we can enable [Kubernetes support](#kubernetes-cli-setup).
...
The `vmType` is set to `vz`instead of `qemu`, `mountType` is set to `virtiofs`, and `rosetta` is turned on. The three configs can boost performance on Apple M1 chip.
...
To enable Kubernetes, ensure the ['kubernetes' config field](#config-colima) is enabled.
...
Additionally, we can customize k3s via the ['k3sArgs' config field](#config-colima). By default, `--disable=traefik` (Ingress controller) is passed the [k3s server](https://docs.k3s.io/cli).

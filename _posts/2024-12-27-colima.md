...
We find 3 containers created for different Kubernetes components (e.g. CoreDNS Cluster DNS).
...
The *colima* [K8s context](https://kubernetes.io/docs/tasks/access-application-cluster/configure-access-multiple-clusters/) is created. Generally speaking, a K8s context is a combination of a cluster, a user (auth) and a optional namespace.
...
Similar to [Docker Context](#switch-docker-engine), we can switch between different K8s contexts by `kubectl config use-context`.

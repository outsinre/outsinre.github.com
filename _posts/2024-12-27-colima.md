...
To enable Kubernetes, we can add the ['kubernetes' config field](#config-colima). As k3s server would consume extra CPU and memory. Please use CLI option `--kubernetes` to dynamically enable Kubernetes on demand.
...
~ $ colima start -p default --kubernetes
```

...
A new [Docker builder](#switch-docker-engine) *colima* is created and automatically activated. Regarding the *error* status of the *default* builder, see [Unix socket file](#unix-socket-file).
...
Run `colima delete` to destroy everything, simulating a fresh installation. However, [colima config](#config-colima) and Docker objects (images/containers included) would be expunged!

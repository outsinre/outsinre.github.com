...
Once started, a new [Docker context](#switch-docker-context) *colima* is created. The context name can be customized in the [config file](#config-colima). The context named *default* is referenced by applications that relies on the [DOCKER_HOST](#dockersock) environment variable.
...
In the [Start colima](#start-colima) section, the [Docker Context](#/2018/04/21/docker-newbie/#docker-context) is automatically switched to the colima Engine. But to definitely let Dokcer CLI communicate with the colima Engine, we should explicitly switch to it.
...
Applications can not find the colima engine as they assume the default pathname `/var/run/docker.sock`. We should customize the via `DOCKER_HOST` and/or `DOCKER_CONTEXT` [Docker environment variables](https://docs.docker.com/reference/cli/docker/#environment-variables).
...
16:29:49 zachary@Zacharys-MacBook-Pro ~

...
The default resources assigned to the Linux VM is 2 CPUs, 2 GiB memory and 100 GiB disk space. We want to increase the CPU and memory a bit.
...
We can customize the default config with the `--edit` CLI option upon [colima start](#start-colima), with the specific config CLI option (e.g. `--dns`), or manually edit the `${COLIMA_HOME}/<profile>/<hostname>.yaml`. For example, the hostname of the colima VM of the `default` profile is `colima`. It can be customized with the `hostname` field.
...
Run `colima delete` to destroy everything, simulating a fresh installation. It would expunge both [colima config](#config-colima) and Docker objects (images/containers included)!

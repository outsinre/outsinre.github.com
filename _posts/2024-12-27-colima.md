...
When connecting to the colima instance, the host IP address would be `192.168.5.2`, and instance IP address would be `192.168.5.1`. By default, the colima VM is not assigned a reachable IP address as it requires root priviledge and has longer startup time. Therefore, we cannot reach the colima instance via `192.168.5.1`. See [the-virtual-machines-ip-is-not-reachable](https://github.com/abiosoft/colima/blob/main/docs/FAQ.md#the-virtual-machines-ip-is-not-reachable).

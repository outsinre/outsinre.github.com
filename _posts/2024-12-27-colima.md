...
To [put it simple](https://github.com/abiosoft/colima/blob/main/docs/FAQ.md), colima [creates a Linux Virtual Machine on macOS](/2018/04/21/docker-newbie/#docker-desktop) based on [Hypervisor](https://developer.apple.com/documentation/hypervisor) and Lima, and then spanws the specified runtime process within.

Containers communicate with the VM instead of macOS host. To make resources on the macOS available to containers, please mount them to the VM first. See the [SSH Agent Forwarding](#ssh-agent-forwarding) example.
...
The default resources assigned to the Linux VM is `2` CPUs, `2` GiB memory and `100` GiB disk space. We want to increase the CPU and memory a bit. Specially, we can set as many CPU (e.g. `8` out `10`) as we can, as the OS is smart enough to schedual the CPU between the colima VM and the system. By default, this rule applies to memory as well (e.g. `30` out `36`). We call this as *ballooning*. However, [due to a macOS bug](https://github.com/lima-vm/lima/issues/2789), the memory ballooning does not work with `vmType: vz`. If the macOS experiences performance issue, reduce the memory size or restart colima.

Additionally, we want to update more fields. The `vmType` is set to `vz`instead of `qemu`, `mountType` is set to `virtiofs`, and `rosetta` is turned on. The three configs can boost performance on Apple M1 chip. Optionally, we can enable [Kubernetes support](#kubernetes-cli-setup).

If you experience any network issue, like cannot connecting to public Internet from within containers or cannot pulling images, please enable `--network-address` and/or `--network-preferred-route`. Once enabled, the VM has an reachable IP address `192.168.64.8`.
...
Ncat: Connected to 192.168.64.8:22.
SSH-2.0-OpenSSH_9.6p1 Ubuntu-3ubuntu13.13
Invalid SSH identification string.
Ncat: 1 bytes sent, 79 bytes received in 0.03 seconds.
```

Upon [colima start](#start-colima), we can customize the default config in three ways.

1. With the `--edit` CLI option
2. With the specific config CLI option (e.g. `--dns`)
3. Manually edit the `${COLIMA_HOME}/<profile>/<hostname>.yaml`. For example, the hostname of the colima VM of the `default` profile is `colima`. It can be customized with the `hostname` field.
...
A new [Docker context](#switch-docker-engine) called *colima* is created and [automatically activated (autoActivate)](#config-colima). The associated `docker.sock` is [mapped from the colima VM to the macOS host](#unit-socket-file).
...
The original Docker official context called *default* is still bound to the [DOCKER_HOST environment variable](#unix-socket-file). We can use this variable to [switch between different engines](#unix-socket-file).

A new [Docker builder](#switch-docker-engine) *colima* is created and automatically activated. Regarding the *error* status of the Docker official *default* builder, see [Unix socket file](#unix-socket-file).
...
The colima VM is assigned an IP address `192.168.5.1` that is [not reachable from the macOS host by default](https://github.com/abiosoft/colima/blob/main/docs/FAQ.md#the-virtual-machines-ip-is-not-reachable). Instead we can connect to the VM via a hidden IP address `192.168.5.2`.
...
Ncat: Version 7.95 ( https://nmap.org/ncat )
Ncat: Connected to 192.168.5.2:22.
SSH-2.0-OpenSSH_9.2p1 Debian-2+deb12u6
Invalid SSH identification string.
Ncat: 1 bytes sent, 76 bytes received in 0.05 seconds.
```
...
2. To update the whole colima (Docker runtime included), refer to [Install colima](#install-colima).

## Clean colima ##

Run `colima k8s stop` and/or `colima stop` to shutdown the colima instance.

Run `colima delete` to destroy everything, including containers, images, the `${COLIMA_HOME}/<profile>/<hostname>.yaml` [config](#config-colima), etc. It simulates a fresh installation.
...
Addtionally, for each container, there are several `docker-proxy` processes spawned by [dockerd](#inspect-colima) when a container is publishing ports, accompanyed by some [iptables rules](/2015/01/29/iptables/) to forward traffic to or from the container within the colima engine.
...
However, applications on the macOS host can not find the colima engine as they assume the default pathname `unix:///var/run/docker.sock`. We should configure the [DOCKER_HOST Docker environment variable](https://docs.docker.com/reference/cli/docker/#environment-variables) to the *colima engine*.
...
2. Bind mound the forwarded SSH agent to your container.
   ...
   If it reports permission error, we should add the write permission `chmod a+w` to the socket file. See [SSH Agent Forwarding](/2018/04/21/docker-newbie/#ssh-agent-forwarding).
...
Occasionally, the `/tmp/colima` mount was borken. We need to manually [restart colima](#start-colima).
...
The *colima* [K8s context](https://kubernetes.io/docs/tasks/access-application-cluster/configure-access-multiple-clusters/) is created. Generally speaking, a K8s context is a combination of a cluster, a user (auth) and an optional namespace. Similar to [Docker Context](#switch-docker-engine), we can switch between different K8s contexts by `kubectl config use-context`.
...
We find 3 Docker containers created for different Kubernetes internal components (e.g. CoreDNS Cluster DNS). You will find each container has an accompanying [pause container](biji/k8s.md#container-types).

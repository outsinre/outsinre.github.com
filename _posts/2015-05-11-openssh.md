...
```
~ $ ssh-keygen -t rsa -b 4096 -P openssh-rsa -C "email@example.com" -o -f ~/.ssh/id_rsa
...
2. It prompts for *passphrase* to encrypt private key *locally*. If someone steal the private key file, he cannot impersonate you to communicate on the Internet. You are encouraged to set a passphrase.
...
2. The default key size is 2048 which is subject to modernized brute force break. Use 4096!
...
2. Use *ssh agent* to cache the passphrase and unlock public-key files automatically.
...
1. `man ssh_config`. The client side config file */etc/ssh/ssh_config* or *~/.ssh/config*. Local host - the host that connect from.
   ...
   *ssh* command supports a variety of different parameters. For example, in order to restrict malicious attempt, you are recommended to change default port number. Other parameters are remote host IP etc. difficult and tedious to remember and input.
   ...
   The *LocalForward* is interesting! 9906 (or 127.0.0.1:9906) is local port and 127.0.0.1:3306 (or x.x.x.x:3306 where x.x.x.x is remote host IP) are remote port. The application first connects to 127.0.0.1:9906 at local host and then is forwarded to 3306 at remote host by SSH tunnel. For example, remote host port 3306 does not accept remote connections except local ones. *ssh -qfCNT tunnel* will do the work! This is a special tunnel! The remote host (x.x.x.x) can be different from *HostName*, i.e. *youtube.com*.

   `-q` causes most warning and diagnostic messages to be suppressed. `-C` requests compression of all data (including stdin, stdout, stderr, and data for forwarded X11, TCP and UNIX-domain connections). Compression is desirable on modem lines and other slow connections, but will only slow down things on fast networks. `-f` requests ssh to go to background just before command execution. `-N` does not execute a remote command.  `-T` disable pseudo-terminal allocation.

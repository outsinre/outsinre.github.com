...
>To speed up TCP connection to VPS, try [KCPtun](https://github.com/xtaci/kcptun).
...
3. Initial server startup
   ...
   ```sh
   # cd /opt/ptproxy/
   # su nobody -s /bin/bash python3.4 ptproxy.py -s server.json &
   ```

   This is the very first launch. `su nobody -s /bin/bash` drop user to *nobody* instead of *root*. Useful information is printed on stdout for client configuration.
...
4. Client startup
   ...
   ```bash
   # cd ~/ptproxy/ptproxy
   # su nobody -s /bin/bash python3.4 ptproxy.py -c client.json >/dev/null &
   ```
...
5. Client startup script
   ...
   start() {
     ...
     if ! pgrep -u nobody -x -f "/usr/bin/python3.4 ${PT_DIR}/ptproxy.py -c ${PT_DIR}/client.json" >/dev/null 2>&1; then
       su nobody -s /bin/bash /usr/bin/python3.4 ${PT_DIR}/ptproxy.py -c ${PT_DIR}/client.json >/dev/null &
     fi
...
4. Run as *nobody*:
   ...
   2. Make *nobody* have *write* access to *pt_state* directory.

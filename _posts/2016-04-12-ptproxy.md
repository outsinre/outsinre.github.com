...
2. Server configuration
   ...
   *ptproxy* server listens on VPS for incoming *ptproxy* client traffic while forwards that to Shadowsocks server.
   ...
   2. Do NOT use bash reserving words in configuration, i.e. `${HOME}`. *ptproxy* does not know about it.
3. Initial server startup

   ```sh
   cd /opt/ptproxy/
   python3.4 ptproxy.py -s server.json &
   ```

   This is the very first launch. Do it in terminal directly. Useful information is printed on stdout for client configuration.
...
5. Real launch by *rc.local*
   ...
   PT_DIR="/opt/ptproxy"
   /usr/bin/python3.4 "${PT_DIR}"/ptproxy.py -s "${PT_DIR}"/server.json &
   ```
...
1. Client configuration
   ...
   The *ptname* and *ptargs* should be set according to the initial server launch output. *iat-mode* is modified if your change it in *obfs4_state.json* before the Real launch.
...
4. Client startup
   ...
   ```bash
   # cd ~/ptproxy/ptproxy
   # python3.4 ptproxy.py -c client.json >/dev/null &
   ```
...
5. Client startup script
   ...
   start() {
     ...
     if ! pgrep -u root -x -f "/usr/bin/python3.4 ${PT_DIR}/ptproxy.py -c ${PT_DIR}/client.json" >/dev/null 2>&1; then
       /usr/bin/python3.4 ${PT_DIR}/ptproxy.py -c ${PT_DIR}/client.json >/dev/null &
     fi
   ...
   stop() {
     ...
     if pgrep  -u root -x -f "/usr/bin/python3.4 ${PT_DIR}/ptproxy.py -c ${PT_DIR}/client.json" >/dev/null 2>&1; then
       pkill -TERM -u root -f "/usr/bin/python3.4 ${PT_DIR}/ptproxy.py -c ${PT_DIR}/client.json"
     fi

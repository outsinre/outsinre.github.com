...
1. Support the *routing* rules based on *domainStrategy* setting.
   ...
   For example, in order to match an IP rule in *routing*, the target domain must be resolved by *dns* first.
2. Resolve domains directed to the *freedom* outbound.

   When a target domain is directed to the *freedom* outbound, it is *directly* accessed and require resolution like normal.

Apart from the its built-in support, V2ray DNS can be used to serve as a system DNS service, similar to Dnsmasq.
...
2. At V2ray client side:
   ...
   2. Set up a *dns* outbound, redirect the query to V2ray server side.
   3. Set up a *routing* rule, routes the query from *dokodemo-door* inbound to *dns* outbound.

We cal also combine V2ray and Dnsmasq. Queries are first sent to local Dnsmasq, then to V2ray *dokodemo-door*.

## External Resources ##

*h2y.dat* is a list of domains desginated for the *routing* section on client sides, focusing on blocked domains and ads domains.

Similar to the built-in domain list, reference to external list can be:

```
"ext:h2y.dat:ad"
```

The *ext* signals it is loaded from 3rd party resources.

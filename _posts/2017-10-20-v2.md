...
To enable global Socks proxy on Windows, the address part in "Windows Settings" should be `http://socks=127.0.0.1`. Alternatively, use the "Internet Options", "Connections", "LAN settings", "Proxy Server", "Advanced" and "Bypass proxy server for local addresses".

Whatever methods you choose, make sure to bypass the remote endpoint address.

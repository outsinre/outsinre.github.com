...
2. Make sure the log directory exists and ownership is correct.
...
A side effect when browsing '/V2', brwoser reports "400 Bad Request" as the backend is a websocket server instead of a bare HTTP server. We may also receive "502 Bad Gateway" as the backend V2 is not started yet.
...
When '/V2' is visted by browser directly, Nginx returns '404.html' page. We can also redirect '/V2' to another page:

```
error_page 400 502 = /;
```

The symbol `=` makes the new response code equal to that of the new page.
...
The *internal* specifies that a given location can only be used for internal requests. For external browser requests, Nginx responds "404 (Not Found)".

Alternatively, try the next method in section [Customize HTTP Header](#customize-http-header).

## Customize HTTP Header ##

Unlike the section [Nginx Template](#nginx-template), we can `proxy_pass` based on customized HTTP headers under the '/' location. This method requires V2 client be 2.40.2 or above.
...
The template above uses customized Host header to redirect request to backend. The customized Host header **must not** be the website domain. Otherwise, all normal HTTP traffic go to backend as well. Usually, we set it to a public domain like *www.amazon.com*. As you will, it can be arbitrary string, like *howareyou*.

However, it rerquires the [evil if directive](https://www.nginx.com/resources/wiki/start/topics/depth/ifisevil/). By the way, manually manipulate the Host header is not recommended as it is usually used to match vhost. If V2 supports self-defined header, then it is far more desirable.
...
This method does not require special *wsSettings* but still uses the *if* directive.

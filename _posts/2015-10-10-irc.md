...
4. Onion certificate.
   ...
   If IRC client supports [**key pinning**](https://freenode.net/kb/answer/chat), we can choose to trust freenode's public key fingerprint instead certificate's fingerprint which benefits us in that the former keep consistent while the later varies in accord with the real freenode server you connect to.
...
Weechat relay protocol is an extension to IRC relay (also known as *IRC proxy*) with full control over remote Weechant relay instance. This section demonstrates the following:
...
3. Add relay
   ...
   3. *ssl* requires certificate support.
...
>Seems without a valid public domain name (`-d`), *certbot* (version 0.12.0 on CentOS 7) refuses to generate `certbot certonly --standalone -d localhost` certificate.

### Getting Certificate from [certbot](https://certbot.eff.org/)
...
~ # ll /etc/letsencrypt/live/irc.example.com
# https://certbot.eff.org/docs/using.html#where-are-my-certificates
~ # certbot renew --dry-run
~ # cerbot renew --renew-hook "systemctl nginx reload"
~ # certbot certonly --standalone --expand -d irc.example.com,cloud.example.com --preferred-challenges tls-sni-01,http --dry-run
```
...
2. If there is a web server currently binding to port 80 or 443, you might need to [turn off web serser](https://certbot.eff.org/docs/using.html#standalone) temprarily.
3. `--renew-hook` tells *nginx* to reload updated certificates. Another way is to pu the hook into *renew* configuration file:

   ```
   # /etc/letsencrypt/renewal/www.example.conf
   [renewalparams]
   renew_hook = systemctl nginx reload
   ```

## Direct connection

The front-end App connects to Weechat relay directly without any intermeddiate parties.

### Let Weechat relay know the Certificate

```bash
~ # cat /etc/letsencrypt/live/relay.example.com/{fullchain,privkey}.pem > relay.pem
~ $ mkdir -p ~/.weechat/ssl
...
```
/set relay.network.ssl_cert_key
/relay sslcertkey
```

After renewal, re-run */relay sslcertkey*.

## Web proxy

Web server (i.e. Nginx) proxifies Weechat relay which let Weechat relay share certificate with web server.

1. Make sure nginx HTTPS server is configured properly.
...
2. Add nginx proxy
   ...
   server {
       ...
       ....
       location /weechat {
	   proxy_pass http://localhost:9001/weechat;     # Change the port to your relay's
...
3. Connect
   1. Add a new relay without SSL option such as */relay add ipv4.weechat 9001*.
   2. Speicify the port to web server's SSL port (i.e. 443) since we are now connecting to proxy instead of direct contact with Weechat relay.

...
# export private key
jim@tux ~ $ gpg --armor --export-secret-keys --export-options backup --output key1-backup.pgp email.gmx.com
```
...
To move personal key pair between computers, you can also use `--import`. Firstly, `--import` the the public key, then `--import` the public key. That's it.

...
6. Get the key ID: the last 64 bits of public key fingerprint.
7. `gpg --list-keys` again to check results.

```bash
jim@laptop ~ $ gpg --gen-key
jim@laptop ~ $ gpg --keyid-format 0xlong --list-keys
```

`--gen-key` is interactive. GnuPG will take several minutes to generate the key pair. You'd better moving the mouse, browsing the web, or having streaming audio in the background will help speed up the process.
...
```bash
jim@laptop ~ $ gpg --armor --export email@gmx.com
```

1. By default, the publick key will be exported to STDOUT.

   `--output name.pub` exports to file. It's recommended to distribute this file along with the *--fingerprint*.
2. `--armor` tells *gpg* to generate ASCII *text* output instead of default *binary*.
3. It not only includes public key string, but others useful information, like key ID, user ID, signatures, fingerprint, sub keys etc. *fingerprint* is usually SHA1 hash. *signature* means the downloaded GnuPG certificate is signed by key owner ('s private key).
...
All these public key servers will exchange keys periodically. Most of the time, you only need to focus on one of them. You can specify your desired key server in *~/.gnupg/gpg.conf* or on command line.

```bash
jim@laptop ~ $ gpg --recv-key key-id
jim@laptop ~ $ gpg --send-key key-id
```

We can specify `--keyserver` in *~/.gnupg/dirmngr.conf*.

>If you cannot connect to kerserver, try [gnupg 2.1.18-1 cannot connect to keyserver](https://bbs.archlinux.org/viewtopic.php?id=222415).
...
## GPG_AGENT_INFO (deprecated since GnuPG v2.1)
...
1. [Backup](https://gist.github.com/chrisroos/1205934)
   ...
   ```bash
   ~ $ gpg --edit-key jim@outlook.com, use *passwd* to set a strong passphrase (opt)
   ...
   2. Backup of public key is unncessary for it's contained in secret key.

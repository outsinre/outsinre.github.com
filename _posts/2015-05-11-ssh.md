...
Your identification has been saved in /home/zachary/.ssh/id_rsa.
Your public key has been saved in /home/zachary/.ssh/id_rsa.pub.
The key fingerprint is:
SHA256:ReC7xNKB2uzib6/q2t9ORrXXVByQMLJ3rV4Vd6AE+Bg rsa-keys
...
1. ***Don't use ssh-dss (DSA) public key algorithm***!!! Refer to [openssh legacy](http://www.openssh.com/legacy.html).
...
```
ssh-copy-id -i ~/.ssh/id_rsa.pub -p 26647 jgray@104.160.32.158
```

Alternatively, we can use:

```
cat ~/.ssh/id_rsa.pub | ssh -p port_num username@remote-host "mkdir -p ~/.ssh && cat >>  ~/.ssh/authorized_keys"
```

1. *remotre-domain-name* can be the IP or domain name of remote host.
2. Both ways achieve the same purpose: append the public key to *authorized_keys*. `ssh-copy-id` automatically creates *~/.ssh/authorized_keys* if it does not exist.
3. Pay attention to `-p` argument of `mkdir`: make parent directories as needed, no error if existing.

No matter which command you chose, you should see something like:

```
The authenticity of host '[104.160.32.158]:26647 ([104.160.32.158]:26647)' can't be established.
RSA key fingerprint is SHA256:mQcHpxXYCQ/yNeSQ7RggCVPrwyXgGJpriD4n8PeoIXM.
Are you sure you want to continue connecting (yes/no)? yes
/usr/bin/ssh-copy-id: INFO: attempting to log in with the new key(s), to filter out any that are already installed
/usr/bin/ssh-copy-id: INFO: 1 key(s) remain to be installed -- if you are prompted now it is to install the new keys
jgray@104.160.32.158's password: 

Number of key(s) added: 1

Now try logging into the machine, with:   "ssh -p '26647' 'jgray@104.160.32.158'"
...
```
ssh -p 26647 jgray@104.160.32.158
Enter passphrase for key '/home/zachary/.ssh/id_rsa': 
...
To use the agent first start it. Just enter "ssh-agent -s" and thats all. This will put you in a bash shell which is spawned by *ssh-agent*. After that you'll need to add your key (file). To do this enter the command *ssh-add*. This will try and add the standard key *identity* to the key manager. To add a key with a different name, enter "ssh-add /location/of/key". After this, the ssh-add program will ask you for your *passphrase*. After that the key is loaded in the key manager *ssh-agent*. 
...
3. Add key file to agent.
    ...
    ```
    ssh-add ~/.ssh/id_rsa
    Enter passphrase for /home/zachary/.ssh/id_rsa: 
    Identity added: /home/zachary/.ssh/id_rsa (/home/zachary/.ssh/id_rsa)
    ```

...
Systemd is a daemon *manager* capable of managing both *system* and *userspace* services.
...
At the very boot of the system, systemd manager runs as the PID 1 for system services:
...
When a user login, a new systemd manager is started for services of that particular account by `user@.service`:
...
~ # systemctl status/enable/start/stop/restart/disable/mask unit@instance.service

...
1. 让多用户共享同一个用户文件夹。
   ...
   打开注册表，`HKEY_CURRENT_USER\Software\Rime\Weasel`里面有一个*string*属性的*RimeUserDir*参数（普通账户没有此参数要新新建一个），把它设置成新的用户文件夹地址，这样不同的Windows账户可以共用同一套配置。

   虽然小狼毫自带的“小狼毫安装选项”可以更改用户文件夹，但是只能管理员账户运行。
2. Windows和Linux没法共用一套配置，因为词库二进制格式不兼容。
3. Rime在Windows和Linux上不区分换行符，`\n`和`\r\n`都支持。

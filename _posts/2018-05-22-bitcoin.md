...
5. We can connect the Bitcoin network through Tor. A bit further, we set our own Bitcoin node reachable through Tor.
...
Up to now, the transaction is not yet confirmed! We should mine 1 block to confirm it.

```bash
user@tux ~ # bitcoin-cli generate 1
```

Similary, we can send bitcoins the other Regtest instance.
